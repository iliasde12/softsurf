...
function displaySongs(songs) {
  ...
  songs.forEach((song, index) => {
    ...
    label.className = "text-white text-sm px-2 py-1 rounded-full";
    label.style.backgroundColor = color;
    label.textContent = mood;
    labelWrapper.appendChild(label);

    const date = document.createElement("span");
    date.className = "hidden md:block text-white/60 text-sm";
    date.textContent = new Date(song.added_at).toLocaleDateString("nl-NL", { day: "numeric", month: "short" });

    const popularity = document.createElement("span");
    popularity.className = "hidden md:block text-[#A0A0C0] text-sm";
