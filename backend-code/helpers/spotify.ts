...
export async function GetTrackSpotify(
  accessToken: string,
  trackId: string,
...
export async function searchSpotifyTracks(query: string, accessToken: string) {
  try {
    ...
    if (!res.ok) {
      console.error("searchSpotifyTracks status:", res.status);
      return [];
    }
    const data = await res.json();
    return data.tracks?.items ?? [];
  } catch (e) {
    console.error("searchSpotifyTracks error:", e);
    return [];
  }
}
...
// api call voor 1 artiest
export async function GetArtistSpotify(accessToken: string, artistId: string) {
  try {
    const response = await fetch(
    ...
    console.error("GetArtistSpotify error:", e);
    return null;
  }
}

// api call voor het aantal albums van een artiest (alleen echte albums, geen singles/compilations)
export async function GetArtistAlbumsCount(
  accessToken: string,
  artistId: string,
): Promise<number> {
  try {
    const response = await fetch(
      `https://api.spotify.com/v1/artists/${artistId}/albums?include_groups=album&limit=50&market=BE`,
      { headers: { Authorization: `Bearer ${accessToken}` } },
    );
    if (!response.ok) return 0;
    const data = await response.json();
    // total bevat het echte aantal, ook al krijg je max 50 items terug
    return data.total ?? data.items?.length ?? 0;
  } catch (e) {
    console.error("GetArtistAlbumsCount error:", e);
    return 0;
  }
}
