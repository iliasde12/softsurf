...
router.get("/search", async (req, res) => {
  ...
  try {
    if (isCollection) {
      ...
      const { fromDB, fromSpotify } = await searchSongsDbSpotify(
        q,
        accessToken,
      );
      res.json({ fromDB, fromSpotify });
...
router.get("/collectie", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Er ging iets mis bij het laden van je collectie" });
  }
});
  ...
  try {
    ...
    if (!response.ok)
      return res
        .status(502)
        .json({
          error: "Shazam API fout: " + response.status,
          detail: rawText,
        });
...
router.get("/song/:id/playable", async (req, res) => {
  ...
  if (ObjectId.isValid(rawId) && rawId.length === 24) {
    ...
    const accessToken = res.locals.spotifyToken;
    if (!accessToken)
      return res.status(401).json({ error: "Geen access token" });
    const spotifySong = await GetTrackSpotify(accessToken, rawId);
    if (!spotifySong)
      return res.status(404).json({ error: "Spotify song niet gevonden" });
    const insertedId = await CreateSong(spotifySong);
    if (!insertedId)
      return res.status(500).json({ error: "Song opslaan mislukt" });
    songId = insertedId;
...
router.post("/playlist/generate", async (req, res) => {
  ...
  try {
    ...
    const nameRes = await generatePlaylistName({
      stemming,
      mixtype,
      tracks: suggestions,
    });
...
router.post("/playlist/create-generated", async (req, res) => {
  ...
  if (!name || !tracks?.length)
    return res
      .status(400)
      .json({ success: false, error: "Naam of nummers ontbreken" });
  try {
    ...
    const coverUrl =
      tracks.find((track: any) => track.album_cover)?.album_cover ?? null;
...
router.get("/game/round", async (req, res) => {
  ...
  try {
    ...
    if (!playable) {
      await CreateSongPlayable(
        song._id,
        song.name,
        artiest,
        song.preview_url ?? undefined,
      );
      playable = await songPlayableCollection.findOne({ songId: song._id });
    }
    if (!playable?.youtubeId)
      return res.status(404).json({ error: "Geen YouTube video gevonden" });
    ...
    const options = [
      ...
      ...wrongSongs.map((s) => ({
        name: s.name,
        artist: s.artists?.[0]?.name ?? "Onbekend",
        correct: false,
      })),
...
router.post("/game/guess", async (req, res) => {
  ...
  if (!sessionId || !songId || !guess)
    return res
      .status(400)
      .json({ error: "sessionId, songId en guess zijn verplicht" });
  try {
    const song = await spotifySongCollection.findOne({
      _id: new ObjectId(songId),
    });
    if (!song) return res.status(404).json({ error: "Nummer niet gevonden" });

    const correct =
      song.name.toLowerCase().trim() === guess.toLowerCase().trim();
    ...
    const session = await gameSessionCollection.findOne({
    ...
    await gameSessionCollection.updateOne(
      { _id: new ObjectId(sessionId) },
      {
        $inc: { total: 1, score: correct ? 1 : 0 },
        $set: { streak: nieuweStreak, updatedAt: new Date() },
      },
    );
...
router.post("/game/session/end", async (req, res) => {
  ...
  if (!sessionId)
    return res.status(400).json({ error: "sessionId is verplicht" });
  try {
    await gameSessionCollection.updateOne(
      { _id: new ObjectId(sessionId) },
      {
...
// haalt 1 track op met alle vergelijk-stats (incl. genre via artiest)
router.get("/track/:id", async (req, res) => {
  const accessToken = res.locals.spotifyToken;
  if (!accessToken) return res.status(401).json({ error: "Geen access token" });

  try {
    const track = await GetTrackSpotify(accessToken, req.params.id);
    if (!track || !track.id)
      return res.status(404).json({ error: "Track niet gevonden" });

    // genre op track bestaat niet → pakken we van de eerste artiest
    const firstArtistId = track.artists?.[0]?.id;
    let genre = "Onbekend";
    if (firstArtistId) {
      const artist = await GetArtistSpotify(accessToken, firstArtistId);
    ...
    res.json({
      id: track.id,
      name: track.name,
      artist: track.artists?.[0]?.name ?? "Onbekend",
    ...
    console.error("GET /track/:id error:", e);
    res.status(500).json({ error: "Er ging iets mis" });
  }
});

// haalt 1 artiest op met alle vergelijk-stats (incl. albums-count via aparte call)
router.get("/artist/:id", async (req, res) => {
  const accessToken = res.locals.spotifyToken;
  if (!accessToken) return res.status(401).json({ error: "Geen access token" });

  try {
    // parallel: artiest + albums-count tegelijk ophalen
    const [artist, albumsCount] = await Promise.all([
      GetArtistSpotify(accessToken, req.params.id),
      GetArtistAlbumsCount(accessToken, req.params.id),
    ]);
    ...
    console.error("GET /artist/:id error:", e);
    res.status(500).json({ error: "Er ging iets mis" });
  }
});

// search voor artiesten (apart endpoint want de bestaande /search doet alleen tracks)
router.get("/search/artist", async (req, res) => {
  ...
  try {
    ...
    console.error("GET /search/artist error:", e);
    res.status(500).json({ error: "Er ging iets mis" });
  }
});
