...
export async function generatePlaylistSuggestions(req: PlaylistRequest): Promise<ClaudePlaylistResponse> {
...
export async function generatePlaylistName({ stemming, mixtype, tracks }: { stemming: string; mixtype: string; tracks: { title: string; artist: string }[] }): Promise<string> {
  const randomSeed = Math.floor(Math.random() * 10000);
