...
export async function searchSpotifyTracks(query: string, accessToken: string) {
  const res = await fetch(`https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=track&limit=10`, { headers: { Authorization: `Bearer ${accessToken}` } });
...
export async function searchTrack(title: string, artist: string, accessToken: string): Promise<SpotifyTrack | null> {
  try {
    const query = `track:${title} artist:${artist}`;
    const res = await fetch(`https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=track&limit=1`, { headers: { Authorization: `Bearer ${accessToken}` } });
    const data = await res.json();
...
export async function searchTracks(suggestions: { title: string; artist: string }[], accessToken: string): Promise<{ suggestion: { title: string; artist: string }; result: SpotifyTrack | null }[]> {
  return Promise.all(
...
export async function getSimilarTracks(
  ...
  const res = await fetch(`https://api.spotify.com/v1/search?q=artist:${encodeURIComponent(artist)}&type=track&limit=10`, { headers: { Authorization: `Bearer ${accessToken}` } });
